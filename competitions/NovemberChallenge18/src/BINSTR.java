...
class BINSTR {
    ...
    static class Trie {
        ...
        Max searchMaxXor(String key, String partialMax) {
            ...
            if (partialMax.length() > keylength && partialMax.length() > length)
                return new Max(-1, "");
            ...
            level = 0;
            if (keylength > length) {
                if(partialMax.length() < keylength)
                    eq = false;
                for (; level < keylength - length; level++) {
                    if(eq) {
                        if (key.charAt(level) < partialMax.charAt(level))
                            return new Max(-1, "");
                        else if (key.charAt(level) > partialMax.charAt(level))
                            eq = false;
                    }
                ...
                if(partialMax.length() < length)
                    eq = false;
                for (; level < length - keylength; level++) {
                    if (pCrawl.children[1] != null) {
                        if (eq && partialMax.charAt(level) == '0')
                            eq = false;
                        next = 1;
                    } else {
                        if (eq && partialMax.charAt(level) == '1')
                            return new Max(-1, "");
                        next = 0;
                    }
                    max.append(next);
            ...
            for (; level < length; level++) {
                shiftPartial = partialMax.length() - length;
                ...
                if ((index == 0 && pCrawl.children[1] != null) || (index == 1 && pCrawl.children[0] != null)) {
                    if (eq && (shiftPartial + level < 0 ? '0' : partialMax.charAt(shiftPartial + level)) == '0')
                        eq = false;
                    next = index == 0 ? 1 : 0;
                    max.append(1);
                } else {
