...
public class B {
    ...
    static int findKth(List<SuffixIndex> si , long k){
        ...
        /*k -= cur.index;
        long c = 0;//k / cur.occurrences - cur.start + (cur.end - cur.length) - 1;
        while(1L * (cur.start - (cur.end - cur.length) + 1 + c) * cur.occurrences <= k){
            k -= 1L * (cur.start - (cur.end - cur.length) + 1 + c) * cur.occurrences;
            if(k < 0)
                System.out.println("problem");
            c++;
        }
        return (int)(cur.end - cur.length + k % (cur.start - (cur.end - cur.length) + 1 + c));*/
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Tree indexed");
        int tl = buildStrTest(txt);
        int idx, q = 200000;//sc.nextInt();
        long g = 0, p, m, k;
        while(q-- > 0){
            /*p = sc.nextLong();
            m = sc.nextLong();*/
            p = (long)(Math.random() * 1000000000) + 1;
            m = (long)(Math.random() * tl) + 1;
            k = (p * g) % m + 1;
            idx = findKth(si, k - 1);
            if((txt.charAt(idx) != strTest.charAt((int)k - 1)))
                System.out.println(txt.charAt(idx) + " " + strTest.charAt((int)k - 1));
            g += txt.charAt(idx);
        }
    }
    ...
    static int buildStrTest(String str){
